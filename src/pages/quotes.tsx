          ...
          <div className="relative px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
            ...
            <div className="relative grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {(quotes ?? []).map((quote: string, index: number) => (
                <div
                  key={index}
                  className="group px-10 py-20 text-center transition duration-300 transform bg-gray-900 rounded shadow-2xl hover:scale-105 hover:bg-white md:shadow-xl hover:shadow-2xl"
                >
...
export const getStaticProps = async () => {
  const quotesData = await client.fetch(
    `*[_type == "post" && categories[0]._ref == $quotesRef][0]{body}`,
    {
